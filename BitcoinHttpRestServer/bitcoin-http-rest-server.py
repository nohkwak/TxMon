...
PORT = 8154
BITCOINRPC = 'http://test:test@127.0.0.1:8332/'

GETBLOCK = "/rest/block"
GETTX = "/rest/tx"
GETADDRESS = "/rest/address"
...
class MyHandler(BaseHTTPRequestHandler):
	def do_GET(self):
		...
		elif os.path.dirname(path) == GETADDRESS:
			self.get_address(os.path.basename(path), format)
		else:
	...
	def get_address(self, addr, format):
		print "Getting address %s (encoding: %s)" % (addr, format)

		try:
			accnt = self.server.btcrpc.getaccount(addr)
			print( accnt )
			tx_list = self.server.btcrpc.listtransactions(accnt)
			print(tx_list)
			if format == "json":
